"""
STAGE 1 (current) — single-branch LSTM/GRU. STAGE 2 (not yet built) — add GAT graph branch fused with LSTM hidden state. STAGE 3 (not yet built) — K-step rollout via teacher forcing. STAGE 4 (optional) — swap backbone for causal-attention Transformer.
"""
...
class AttackWorldModel(nn.Module):
    def __init__(self, config: ModelConfig):
        ...
        self.backbone = recurrent(config.input_size, config.hidden_size, config.num_layers,
    ...
    def forward(self, sequence: torch.Tensor) -> dict[str, torch.Tensor]:
        ...
        hidden = output[:, -1, :]
        return {"next_state_pred": self.next_state_head(hidden),
                "infiltration_logit": self.infiltration_head(hidden).squeeze(-1),
                "stage_logits": self.stage_head(hidden)}

    def compute_loss(self, outputs: dict[str, torch.Tensor], next_state: torch.Tensor,
                     infiltration: torch.Tensor, stage: torch.Tensor,
                     class_weights: torch.Tensor | None = None) -> dict[str, torch.Tensor]:
        mse = nn.functional.mse_loss(outputs["next_state_pred"], next_state)
        bce = nn.functional.binary_cross_entropy_with_logits(outputs["infiltration_logit"], infiltration.float())
    ...
    def predict(self, window_sequence: np.ndarray | torch.Tensor) -> dict:
        ...
        return {"next_state_pred": outputs["next_state_pred"][0].cpu().tolist(),
                "infiltration_prob": float(infiltration_prob),
                "stage_probabilities": stage_probabilities.cpu().tolist(),
                "predicted_stage": predicted_stage, "confidence": confidence}
