    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <StatTile
          ...
          sub="CyberAuditLog.sol"
          color="indigo"
        />
      </div>

      {/* Detection Engine Status Banner */}
      <div className="bg-slate-900/60 p-5 rounded-2xl border border-slate-800 backdrop-blur-md flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-xl">
        <div className="flex items-center gap-3">
          <div className="p-3 bg-indigo-500/10 border border-indigo-500/30 rounded-xl text-indigo-400">
            <Zap className="w-6 h-6" />
          </div>
          <div>
            <h3 className="text-base font-bold text-white font-heading">Multi-Confirm Anomaly Detection Gate</h3>
            <p className="text-xs text-slate-400 mt-0.5 font-mono">
              Enforces Z-Score Frequency Spikes + Isolation Forest Outliers + Behavioral Rule Heuristics. Requires ≥2 algorithms to agree before flagging an anomaly.
            </p>
          </div>
        </div>
